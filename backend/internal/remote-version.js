...
const VERSION_URL = "https://api.github.com/repos/snxRCS/better-npm/releases/latest";
const VERSION_CHECK_ENABLED = process.env.VERSION_CHECK_ENABLED !== "false";
...
const internalRemoteVersion = {
	 ...
	 * Can be disabled via VERSION_CHECK_ENABLED=false environment variable.
	...
	get: async () => {
		...
		if (!VERSION_CHECK_ENABLED) {
			debug(logger, "Version check is disabled by environment variable");
			return {
	...
	fetchUrl: (url) => {
		...
		const headers = {
			"User-Agent": `BetterNPM v${pjson.version}`,
		};
