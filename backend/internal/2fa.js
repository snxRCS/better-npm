...
const internal2fa = {
	...
	isEnabled: async (userId) => {
		const auth = await internal2fa.getUserPasswordAuthSafe(userId);
	...
	/**
	 * Safe version: returns null instead of throwing if no password auth exists.
	 * Used for LDAP users who only have type "ldap" auth records.
	 */
	getUserPasswordAuthSafe: async (userId) => {
