...
const DEFAULT_AVATAR = avatar.generateAvatar("admin@example.com", "Admin");
...
const internalUser = {
	...
	create: async (access, data) => {
		...
		data.avatar = avatar.generateAvatar(data.email, data.name);
	...
	update: (access, data) => {
		...
		return access
			...
			.then((user) => {
				...
				data.avatar = avatar.generateAvatar(data.email || user.email, data.name || user.name);
