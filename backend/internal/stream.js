...
/**
 * Check if an incoming port is already in use by another stream
 * @param {Number} port - The incoming port to check
 * @param {Number} [excludeId] - Stream ID to exclude (for updates)
 * @returns {Promise<void>} - Throws if port is already in use
 */
const checkPortAvailability = async (port, excludeId) => {
	if (!port) return;
	const query = streamModel
		.query()
		.where("incoming_port", port)
		.where("is_deleted", 0);
	if (excludeId) {
	...
	if (existing) {
		throw new errs.ValidationError(`Port ${port} is already in use by another stream`);
	}
};
...
const internalStream = {
	...
	create: (access, data) => {
		...
		return access
			...
			.then(async (/*access_data*/) => {
				// Validate that the incoming port is not already in use
				await checkPortAvailability(data.incoming_port);
	...
	update: (access, data) => {
		...
		return access
			...
			.then(async (/*access_data*/) => {
				// Validate that the incoming port is not already in use by another stream
				if (thisData.incoming_port) {
					await checkPortAvailability(thisData.incoming_port, thisData.id);
				}
