...
const getAuthSourceBadge = (event: AuditLog) => {
	if (event.objectType !== "user") return null;
	const authSource = event.meta?.authSource || event.meta?.auth_source;
	if (authSource === "ldap") {
		return <span className="badge bg-purple-lt ms-1">LDAP</span>;
	}
	if (authSource === "internal") {
		return <span className="badge bg-blue-lt ms-1">Internal</span>;
	}
	return null;
};
