...
function SSOLoadingState() {
	return (
		<>
			<h2 className="h2 text-center mb-4"><T id="login.sso" /></h2>
			<div className="text-center">
				<div className="spinner-border text-azure mb-3" role="status" />
				<p className="text-secondary"><T id="login.sso.authenticating" /></p>
			</div>
		</>
	);
}
...
export default function Login() {
	const { twoFactorChallenge, ssoLoading } = useAuthState();
	...
	const renderContent = () => {
		if (ssoLoading) return <SSOLoadingState />;
		if (twoFactorChallenge) return <TwoFactorForm />;
		return <LoginForm />;
	};
